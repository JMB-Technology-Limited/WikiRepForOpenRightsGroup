 ...
 */
class GetPublishedRecordsFieldTypeDateWithDataBaseTest extends BaseTestWithDataBase
{
    ...
    public function testDate1() {
        ...
        $this->assertNull($records[0]->getFieldValue('date'));
        ...
        $this->assertEquals(1, count($records));
        $this->assertEquals('r1', $records[0]->getPublicId());
        $this->assertEquals('DirectokiBundle\InternalAPI\V1\Model\FieldValueDate', get_class($records[0]->getFieldValue('date')));
        $this->assertEquals('2016-01-01', $records[0]->getFieldValue('date')->getValue()->format('Y-m-d'));
